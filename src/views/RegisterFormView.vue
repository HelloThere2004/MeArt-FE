<template>
  <div class="register-form card p-4 shadow-sm border-0">
    <h4 class="fw-bold mb-3 text-center">Đăng Ký Tư Vấn / Học Thử</h4>
    <p class="text-muted text-center mb-4">
      Để lại thông tin, Mê Art sẽ liên hệ với bạn sớm nhất nhé!
    </p>
    ...
    <form v-else @submit.prevent="submitForm">
      ...
      <div class="mb-4">
        ...
        <textarea
          ...
          class="form-control"
          rows="2"
          placeholder="Bạn muốn hỏi thêm gì không?"
        ></textarea>
      </div>

      <button
        type="submit"
        class="btn custom-btn-blue text-white w-100 fw-bold"
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        const { error } = await supabase.from('registerForm').insert([this.formData])
