<template>
  <div class="admin-outstanding">
    <h3 class="mb-4" style="color: #1e40af">Quản lý Học Sinh Ưu Tú</h3>

    <div class="card mb-4 p-4 border-0 shadow-sm add-card">
      <h5 class="mb-3" style="color: #1e40af">+ Thêm Gương Mặt Mới</h5>
      <form @submit.prevent="addStudent" class="row g-3">
        ...
        <div class="col-md-6">
          <input
            v-model="newStudent.achievement"
            type="text"
            class="form-control"
            placeholder="Thành tích (VD: Thủ khoa Kiến Trúc)..."
            required
          />
        </div>
        <div class="col-12">
          <textarea
            v-model="newStudent.content"
            class="form-control"
            rows="3"
            placeholder="Đôi lời cảm nhận hoặc quá trình học tập..."
            required
          ></textarea>
        </div>
        <div class="col-md-8">
        ...
        <div class="col-md-4">
          <button type="submit" class="btn btn-primary w-100 fw-bold" :disabled="isUploading">
    ...
    <div class="table-responsive">
      <table
        class="table table-hover align-middle bg-white shadow-sm"
        style="table-layout: fixed; width: 100%"
      >
        <thead style="background-color: #1e40af; color: #ffffff">
          <tr>
            <th style="width: 15%; color: #ffffff">Ảnh</th>
            <th style="width: 20%; color: #ffffff">Học sinh</th>
            <th style="width: 20%; color: #ffffff">Thành tích</th>
            <th style="width: 35%; color: #ffffff">Nội dung</th>
            <th style="width: 10%; text-align: center; color: #ffffff">Xóa</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="student in students" :key="student.id">
            ...
            <td>
              <span class="badge bg-primary">{{ student.achievement }}</span>
            </td>
            <td style="white-space: normal; word-wrap: break-word">{{ student.content }}</td>
            <td class="text-center">
              <button
                class="btn btn-sm btn-outline-danger"
                @click="deleteStudent(student.id, student.image_url)"
              >
